...
def create_field_transform(
    ...
    - If both `b_field` and `e_field` are provided, it creates an orthonormal basis
      (v_B, v_E_perp, v_BxE_perp) and projects the velocity onto it.
    ...
    if e_field is not None and e_field.shape != (3,):
        raise ValueError(f"e_field must be a 3-component vector, but got shape {e_field.shape}")
    ...
    def field_transform(data: np.ndarray) -> Tuple[np.ndarray, List[str]]:
        ...
        if e_field is None:
            ...
            # Get perpendicular component magnitude using Pythagorean theorem
            ...
            if np.isclose(norm_E_perp, 0):
                raise ValueError("E field is parallel to B field, cannot form an orthonormal basis.")
            ...
            new_names = ["v_B", "v_E_perp", "v_BxE_perp"]
        ...
        keep_indices = [i for i in range(data.shape[1]) if i not in velocity_indices]
        ...
        kept_names = [name for i, name in enumerate(original_component_names) if i in keep_indices]
